...
const VoiceGiftDetail = () => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (recordFile) {
        const recordUrl = await uploadFile(recordFile, "records");
    ...
    <Box>
      ...
      <Box display="none">
        {audio && (
          ...
          >
            <source src={audio.url} alt="audio" />
          </audio>
        )}
        {recordFile && (
          <audio
